...
const sanitizeHex = (c: unknown, fb = '#111827') =>
  isString(c) && /^#[\da-fA-F]{6}$/.test(c.trim()) ? c.trim() : fb;
...
const fontStackOf = (f: FontFamily) =>
  f === 'Inter'
    ...
    : f === 'Roboto'
    ? 'Roboto, ui-sans-serif, system-ui, -apple-system, Segoe UI, Arial'
    : f === 'Poppins'
    ? 'Poppins, ui-sans-serif, system-ui, -apple-system, Segoe UI, Arial'
    ...
    shell: {
      ...
      tabs: tabsUnknown.map((t) => {
        ...
        const iconKind: IconKind =
          kindRaw === 'image' || kindRaw === 'emoji' || kindRaw === 'builtin'
            ? (kindRaw as IconKind)
            : 'builtin';
        return {
...
export default function ThemeEditor() {
  ...
  const setTab = <K extends keyof Tab>(i: number, k: K, v: Tab[K]) =>
    setDraft((d) => {
      const tabs = [...d.shell.tabs];
      tabs[i] = { ...tabs[i], [k]: v };
      return { ...d, shell: { ...d.shell, tabs } };
    });
  const addTab = () =>
    setDraft((d) => ({
      ...d,
      shell: { ...d.shell, tabs: [...d.shell.tabs, { label: 'Yeni', url: '/', iconKind: 'emoji', emoji: '✨' }] },
    }));
  const removeTab = (i: number) =>
    setDraft((d) => ({ ...d, shell: { ...d.shell, tabs: d.shell.tabs.filter((_, idx) => idx !== i) } }));

  const saveDraft = () => {
    localStorage.setItem('theme_draft', JSON.stringify({ ...draft, site: { url: webUrl } }));
    alert('Taslak kaydedildi.');
  };
  const publishDraft = () => {
    localStorage.setItem('theme_published', JSON.stringify({ ...draft, site: { url: webUrl } }));
  ...
  const resetAll = () => {
    ...
    setWebUrl(initialDraft.site.url);
    setFrameKey((k) => k + 1);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${shell.primary}, ${shell.secondary})` }}
            />
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={saveDraft}
              className="h-9 rounded-xl border px-4 text-sm font-semibold hover:bg-slate-50 border-slate-300"
            >
              Taslağı Kaydet
            </button>
            <button
              ...
              style={{ background: shell.primary, color: contrastOn(shell.primary) }}
            >
              Yayınla
            </button>
            <button
              onClick={resetAll}
              className="h-9 rounded-xl border px-3 text-sm font-semibold hover:bg-red-50 border-slate-300 text-red-700"
            >
              Sıfırla
            </button>
          </div>
        ...
        <div className="space-y-8">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto] items-end">
              ...
              <button onClick={tryLoad} className="h-11 rounded-xl border border-slate-300 px-4 hover:bg-slate-50">
                Yenile
              </button>
              <a
                href={normalizeUrl(webUrl) || '#'}
                target="_blank"
                className="h-11 rounded-xl border border-slate-300 px-4 hover:bg-slate-50 flex items-center justify-center"
              >
                Yeni sekmede aç
              </a>
            </div>
            <p className="mt-2 text-xs text-slate-500">
              Bazı siteler iframe’i engeller (X‑Frame‑Options/CSP). Engellenirse telefonda uyarı gösterilir.
            </p>
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2">
              <div>
                ...
                <input
                  value={shell.appName}
                  onChange={(e) => setShell('appName', e.target.value)}
                  className="mt-1 w-full h-11 rounded-xl border border-slate-300 px-3"
              ...
              <ColorPicker
                ...
                value={shell.secondary}
                onChange={(v) => setShell('secondary', v)}
              />
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={shell.bannerEnabled}
                  onChange={(e) => setShell('bannerEnabled', e.target.checked)}
                />
                <span>Aktif</span>
              </label>
            </div>
              ...
              <div className="mt-4 grid gap-5">
                <div className="grid md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      value={shell.noticeText ?? ''}
                      onChange={(e) => setShell('noticeText', e.target.value)}
                      className="mt-1 w-full h-11 rounded-xl border border-slate-300 px-3"
                  ...
                  <div>
                    ...
                    <input
                      value={shell.bannerLink ?? ''}
                      onChange={(e) => setShell('bannerLink', e.target.value)}
                      className="mt-1 w-full h-11 rounded-xl border border-slate-300 px-3"
                      placeholder="https://..."
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    value={shell.bannerImage ?? ''}
                    onChange={(e) => setShell('bannerImage', e.target.value)}
                    className="mt-1 w-full h-11 rounded-xl border border-slate-300 px-3"
                    placeholder="https://..."
                  />
                  <div className="mt-2">
                  ...
                  <div className="mt-3 flex flex-wrap gap-3">
                    {SAMPLE_BANNERS.map((src) => (
                      <button
                        key={src}
                        type="button"
                        onClick={() => setShell('bannerImage', src)}
                        className={cn(
                          'relative h-16 w-28 overflow-hidden rounded-xl border border-slate-300',
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  <ColorPicker
                    label="Duyuru Arka Plan"
                    value={shell.noticeBg ?? '#FFF7ED'}
                    onChange={(v) => setShell('noticeBg', v)}
                  />
                  <ColorPicker
                    label="Duyuru Yazı Rengi"
                    value={shell.noticeTextColor ?? '#7C2D12'}
                    onChange={(v) => setShell('noticeTextColor', v)}
                  />
                </div>
              </div>
            )}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <button onClick={addTab} className="h-9 rounded-lg border border-slate-300 px-3 text-sm hover:bg-slate-50">
                Sekme Ekle
              </button>
            </div>
              ...
              {shell.tabs.map((t, i) => (
                <div key={i} className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <SegButton active={t.iconKind === 'builtin'} onClick={() => setTab(i, 'iconKind', 'builtin')}>
                        Built‑in
                      </SegButton>
                      <SegButton active={t.iconKind === 'emoji'} onClick={() => setTab(i, 'iconKind', 'emoji')}>
                        Emoji
                      </SegButton>
                      <SegButton active={t.iconKind === 'image'} onClick={() => setTab(i, 'iconKind', 'image')}>
                        Resim URL
                      </SegButton>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {EMOJI_SET.map((ej) => (
                          <button
                            ...
                            className={cn(
                              'h-10 w-10 rounded-lg border border-slate-300 flex items-center justify-center text-lg',
                    ...
                    {t.iconKind === 'image' && (
                      <div className="grid gap-2">
                        <input
                          ...
                          className="h-11 rounded-xl border border-slate-300 px-3"
                          placeholder="https://cdn.site.com/icon.png (32x32)"
                        />
                  ...
                  <div className="md:col-span-2 flex justify-end">
                    <button onClick={() => removeTab(i)} className="h-10 rounded-lg border border-slate-300 px-3 hover:bg-slate-50">
                      Sekmeyi Sil
                    </button>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mx-auto w-[390px]">
              <div
                className="relative mx-auto h-[844px] w-[390px] rounded-[48px] border border-slate-300 bg-slate-900/90 shadow-[0_10px_40px_rgba(0,0,0,.25)] overflow-hidden"
              ...
              >
                ...
                <div className="absolute inset-1 rounded-[40px] overflow-hidden bg-white">
                  {/* --- TEK SARMALAYICI: safe-top/bottom padding + iframe full --- */}
                  <div
                    className="absolute inset-0"
                    style={
                      {
                        ['--safe-top' as unknown as string]: `${SAFE_TOP}px`,
                        ['--safe-bottom' as unknown as string]: `${SAFE_BOTTOM}px`,
                  ...
                  {shell.bannerEnabled && (
                    <a
                      ...
                      style={{
                        ...
                        borderColor: hexToRgba('#000', 0.15),
                  ...
                  {/* Alt menü (zorunlu) */}
                  <div
                    className="absolute inset-x-3 bottom-3 z-40 rounded-2xl border bg-white/95 backdrop-blur px-2 py-2"
                    style={{ borderColor: hexToRgba('#000', 0.12), fontFamily: 'var(--ap-font)', color: 'var(--ap-text)' }}
                  >
                    <div className="grid grid-cols-3 gap-2">
                      {shell.tabs.map((t, i) => (
                        <a
                          ...
                          style={{ borderColor: hexToRgba('#000', 0.08) }}
                        >
              ...
              <div className="mt-2 text-xs text-slate-500">
                Boyut: 390×844 (iPhone 14 Pro). Tema yalnızca bu önizlemeye uygulanır.
              </div>
            </div>
          </section>
        </div>
...
function FontPicker(props: { label: string; value: FontFamily; onChange: (v: FontFamily) => void }) {
  ...
  return (
    <div>
      <label className="text-sm">{label}</label>
      <select value={value} onChange={(e) => onChange(e.target.value as FontFamily)} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3">
        {FONT_OPTIONS.map((f) => (
